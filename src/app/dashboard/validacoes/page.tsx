...
function EditarValidacaoModal({ group, onClose, onSaved, descMap }: { group: WeekGroup; onClose: () => void; onSaved: () => void; descMap: Record<string, string> }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (countAg > 0 || hasAgDivs) {
        if (sessionAgId) {
          await supabase.from("validation_sessions").update({ session_date: weekStart, week_number: parseInt(weekNumber), year: parseInt(year), week_start: weekStart, validated_count: countAg, notes: notes || null }).eq("id", sessionAgId);
        } else {
          const { data: nAg } = await supabase.from("validation_sessions").insert({ session_date: weekStart, week_number: parseInt(weekNumber), year: parseInt(year), week_start: weekStart, week_end, company: "AG", type: group.type, validated_count: countAg, notes: notes || null }).select().single();
          sessionAgId = nAg?.id;
        }
      }
      ...
      if (countBr > 0 || hasBrDivs) {
        if (sessionBrId) {
          await supabase.from("validation_sessions").update({ session_date: weekStart, week_number: parseInt(weekNumber), year: parseInt(year), week_start: weekStart, validated_count: countBr, notes: notes || null }).eq("id", sessionBrId);
        } else {
          const { data: nBr } = await supabase.from("validation_sessions").insert({ session_date: weekStart, week_number: parseInt(weekNumber), year: parseInt(year), week_start: weekStart, week_end, company: "BR", type: group.type, validated_count: countBr, notes: notes || null }).select().single();
          sessionBrId = nBr?.id;
        }
      }
              ...
              <div className="max-h-60 overflow-y-auto custom-scrollbar divide-y divide-white/[0.03]">
                {rows.map(row => (
                  <div key={row.key} className="grid items-center px-3 py-2 hover:bg-white/[0.01] gap-1.5"
                    ...
                    <div className="relative">
                      <button type="button" onClick={() => setActiveDropdown(activeDropdown?.key === row.key && activeDropdown?.field === 'company' ? null : { key: row.key, field: 'company' })} className="w-full bg-[#121622] border border-white/5 rounded-xl px-2 py-1.5 text-[11px] font-bold text-white/80 outline-none hover:border-white/10 transition-all flex items-center justify-between gap-1">
                        <span className="w-full text-center">{row.company}</span>
                        <ChevronDown size={10} className="text-white/30 shrink-0" />
                      </button>
                      {activeDropdown?.key === row.key && activeDropdown?.field === 'company' && (
                    ...
                    <input type="text" value={row.code} onChange={e => { const code = e.target.value; updateRow(row.key, "code", code); const clean = code.trim().toUpperCase(); if (descMap[clean]) updateRow(row.key, "description", descMap[clean]); }} className={cn(rowField, "px-2 py-1.5 rounded-xl font-mono text-white/80")} />
                    <input type="text" value={row.treatment} placeholder="Tratamento..." onChange={e => updateRow(row.key, "treatment", e.target.value)} className={cn(rowField, "px-2 py-1.5 rounded-xl text-white/80")} />
                    
                    <div className="relative">
                      <button type="button" onClick={() => setActiveDropdown(activeDropdown?.key === row.key && activeDropdown?.field === 'resolved' ? null : { key: row.key, field: 'resolved' })} className="w-full bg-[#121622] border border-white/5 rounded-xl px-2 py-1.5 text-[11px] font-bold outline-none hover:border-white/10 transition-all flex items-center justify-between gap-1">
...
function DetailView({ v, onBack, descMap }: { v: WeekGroup; onBack: () => void; descMap: Record<string, string> }) {
  ...
  async function handleCapture() {
    ...
    try {
      ...
      link.download = `auditoria_semana_${v.week_number}.png`;
  ...
  return (
    <div className="min-h-full space-y-6">
      <div style={{ position: "fixed", left: -9999, top: 0, zIndex: -1 }}>
        <ShareCard cardRef={cardRef} items={v.divergences} weekInfo={`Semana ${v.week_number}`} />
      </div>
      ...
      <div className="flex items-center justify-between">
        <button onClick={onBack} className="flex items-center gap-2 text-white/40 hover:text-white transition-colors group">
          ...
          <span className="text-[11px] font-bold uppercase tracking-widest">Voltar para Lista</span>
        </button>
        <button onClick={handleCapture} disabled={capturing} className="flex items-center gap-2 bg-white/5 border border-white/10 hover:bg-white/10 text-white text-[11px] font-bold uppercase tracking-wider px-5 py-2.5 rounded-xl transition-all">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex items-center justify-between mb-8">
              <div>
                <div className="flex items-center gap-3 mb-1">
                  <h2 className="text-2xl font-bold uppercase tracking-tighter">Semana {v.week_number}</h2>
                  <span className="text-[10px] font-extrabold bg-blue-500/10 text-blue-400 border border-blue-500/20 px-2.5 py-1 rounded-full uppercase tracking-widest">{v.type}</span>
                </div>
                <p className="text-[11px] text-white/30 font-bold uppercase tracking-widest">{fmtFullDate(v.week_start)} até {fmtFullDate(v.week_end)}</p>
              </div>
              <div className="text-right">
                <span className="block text-[10px] font-extrabold text-white/20 uppercase tracking-widest mb-1">Acurácia da Semana</span>
                <span className={cn("text-3xl font-light tabular-nums", accuracy >= 95 ? "text-emerald-400" : "text-amber-400")}>{accuracy.toFixed(1)}%</span>
              </div>
            </div>

            <div className="grid grid-cols-3 gap-4">
              {[
                { label: "Auditadas", val: totalVal, color: "text-white/60" },
                { label: "Divergências", val: totalDivs, color: "text-red-400" },
                { label: "Corretas", val: totalVal - totalDivs, color: "text-emerald-400" },
              ].map(s => (
                <div key={s.label} className="bg-white/[0.02] border border-white/5 rounded-2xl p-4">
                  <span className="block text-[9px] font-bold text-white/20 uppercase tracking-widest mb-1">{s.label}</span>
                  <span className={cn("text-xl font-bold font-mono", s.color)}>{s.val}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[#0D1117] border border-white/5 rounded-[32px] overflow-hidden">
            <div className="px-8 py-6 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
              <h3 className="text-[11px] font-extrabold uppercase tracking-widest text-white/40">Detalhamento das Divergências</h3>
              <span className="text-[10px] font-bold bg-red-500/10 text-red-400 px-2.5 py-1 rounded-full">{totalDivs} registros</span>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead>
                  <tr className="border-b border-white/5 text-[9px] font-extrabold uppercase tracking-widest text-white/20">
                    <th className="px-8 py-4">Posição</th>
                    <th className="px-8 py-4">Código</th>
                    <th className="px-8 py-4">Empresa</th>
                    <th className="px-8 py-4 text-center">Sistema</th>
                    <th className="px-8 py-4 text-center">Físico</th>
                    ...
                    <tr key={i} className="hover:bg-white/[0.01] transition-colors">
                      ...
                      <td className="px-8 py-4">
                        <div className="flex flex-col">
                          <span className="text-[12px] font-bold text-white/80">{d.code}</span>
                          <span className="text-[10px] text-white/30 truncate max-w-[200px]">{descMap[d.code] || "—"}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-8 py-4">
                        <span className={cn("text-[9px] font-extrabold uppercase tracking-widest px-2 py-0.5 rounded-full", d.resolved ? "text-emerald-400 bg-emerald-500/10" : "text-amber-400 bg-amber-500/10")}>{d.resolved ? "Resolvido" : "Pendente"}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="space-y-6">
          <div className="bg-[#0D1117] border border-white/5 rounded-[32px] p-6">
            <h3 className="text-[10px] font-extrabold uppercase tracking-widest text-white/20 mb-6">Divisão por Empresa</h3>
            <div className="space-y-6">
              {[
                { label: "AG", val: valAg, divs: v.divergences.filter(d => d.company === "AG").length, color: "text-blue-400", bg: "bg-blue-400" },
                { label: "BR", val: valBr, divs: v.divergences.filter(d => d.company === "BR").length, color: "text-emerald-400", bg: "bg-emerald-400" },
              ].map(c => {
                const acc = c.val > 0 ? ((c.val - c.divs) / c.val) * 100 : 100;
                return (
                  <div key={c.label} className="space-y-3">
                    <div className="flex justify-between items-end">
                      <span className={cn("text-sm font-black", c.color)}>{c.label}</span>
                      <span className="text-[11px] font-bold text-white/60">{acc.toFixed(1)}%</span>
                    </div>
                    <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                      <div className={cn("h-full rounded-full transition-all duration-1000", c.bg)} style={{ width: `${acc}%` }} />
                    </div>
                    <div className="flex justify-between text-[9px] font-bold uppercase tracking-widest text-white/20">
                      <span>{c.val} auditadas</span>
                      <span>{c.divs} divergências</span>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="bg-blue-600 rounded-[32px] p-6 text-white relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:scale-110 transition-transform duration-500"><ShieldAlert size={80} /></div>
            <h3 className="text-[10px] font-black uppercase tracking-widest mb-4 opacity-60">Observações da Auditoria</h3>
            <p className="text-sm font-medium leading-relaxed mb-6">{v.notes || "Nenhuma observação registrada para este período de auditoria."}</p>
            <div className="flex items-center gap-3 pt-6 border-t border-white/10">
              <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center"><CheckCircle2 size={14} /></div>
              <span className="text-[10px] font-bold uppercase tracking-widest opacity-60">Validado pelo Sistema</span>
            </div>
          </div>
        </div>
      </div>
...
function EditarTratativaModal({ group, onClose, onSaved }: { group: ConsolidatedGroup; onClose: () => void; onSaved: () => void }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      const { error: err } = await supabase.from("divergences").update({ treatment, observation, resolved }).in("id", ids);
      if (err) throw err;
      onSaved(); onClose();
    } catch (err: unknown) {
      ...
      <div className="bg-[#0A0D14] border border-white/10 rounded-[28px] w-full max-w-xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between px-8 py-6 border-b border-white/5 bg-[#07090F]">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-base font-bold text-white uppercase tracking-wider">Tratativa de Divergência</h3>
              <p className="text-[11px] text-white/40 mt-0.5 font-medium tracking-wide">Defina a ação corretiva e o status final</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 rounded-xl text-white/30 hover:text-white/85 hover:bg-white/5 transition-colors"><X size={18} /></button>
        </div>
        ...
        <div className="p-8 space-y-6">
          <div className="grid grid-cols-2 gap-4 bg-white/[0.02] border border-white/5 rounded-2xl p-4">
            <div>
              ...
              <span className="text-[13px] font-mono font-bold text-white">{group.position}</span>
            </div>
            <div>
              <span className="block text-[10px] uppercase tracking-wider text-white/40 mb-1">Código</span>
              <span className="text-[13px] font-mono font-bold text-white">{group.code}</span>
            </div>
          ...
          <div>
            ...
            <div className="relative">
              <button type="button" onClick={() => setStatusOpen(o => !o)} className="w-full bg-[#121622] border border-white/5 rounded-2xl px-4 py-3 text-[13px] outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/30 transition-all flex items-center justify-between gap-3 hover:border-white/10">
                <div className="flex items-center gap-2.5">
                  {resolved ? (
                    <><span className="w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_6px_rgba(52,211,153,0.6)]" /><span className="text-emerald-400 font-bold">Resolvido</span></>
                  ) : (
                    <><span className="w-2 h-2 rounded-full bg-amber-400 shadow-[0_0_6px_rgba(251,191,36,0.6)]" /><span className="text-amber-400 font-bold">Pendente</span></>
                  )}
                </div>
              ...
              {statusOpen && (
                <div className="absolute top-full left-0 right-0 mt-1.5 bg-[#121622] border border-white/10 rounded-2xl shadow-2xl overflow-hidden z-50">
                  <button type="button" onClick={() => { setResolved(false); setStatusOpen(false); }} className={cn("w-full flex items-center gap-3 px-4 py-3.5 text-[13px] font-bold transition-all hover:bg-white/5", !resolved ? "bg-amber-500/10 text-amber-400" : "text-white/50")}>
                    <span className="w-2 h-2 rounded-full bg-amber-400 shadow-[0_0_6px_rgba(251,191,36,0.5)]" />Pendente
                  </button>
                  <div className="h-px bg-white/5" />
                  <button type="button" onClick={() => { setResolved(true); setStatusOpen(false); }} className={cn("w-full flex items-center gap-3 px-4 py-3.5 text-[13px] font-bold transition-all hover:bg-white/5", resolved ? "bg-emerald-500/10 text-emerald-400" : "text-white/50")}>
                    <span className="w-2 h-2 rounded-full bg-emerald-400 shadow-[0_0_6px_rgba(52,211,153,0.5)]" />Resolvido
                  </button>
                </div>
              )}
            </div>
        ...
        <div className="flex items-center justify-end gap-3 px-8 py-6 border-t border-white/5 bg-[#07090F]">
          ...
          <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-[12px] font-bold uppercase tracking-wider transition-all disabled:opacity-50">
            {saving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />}{saving ? "Salvando..." : "Salvar Tratativa"}
          </button>
        </div>
      </div>
    </div>
...
export default function ValidacoesPage() {
  ...
  const deleteWeekGroup = async (v: WeekGroup, e: React.MouseEvent) => {
    ...
    try {
      ...
      if (ids.length > 0) await supabase.from("validation_sessions").delete().in("id", ids);
      fetchValidations();
  ...
  allDivergencesList.forEach(d => {
    ...
    if (!consolidatedMap[groupKey]) {
      consolidatedMap[groupKey] = { key: groupKey, position: d.position, code: d.code, resolved: isResolved, treatment: d.treatment ?? "", observation: d.observation ?? "", company: d.company, items: [] };
    }
    ...
    <>
      ...
      <div className="min-h-full">
        <div className="w-full max-w-full px-6 md:px-10 lg:px-14 py-8 mx-auto space-y-6">
          <div className="flex items-center justify-between">
            <span className="text-white/60 font-bold uppercase tracking-widest text-[11px] flex items-center gap-2"><ScanBarcode size={14} className="text-blue-500 animate-pulse" /> Auditoria Geral de Picking</span>
            <button onClick={() => setShowModal(true)} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-white text-[11px] font-bold uppercase tracking-wider px-5 py-2.5 rounded-xl transition-all"><Plus size={14} /> Nova Validação</button>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              <div className="bg-[#0D1117] border border-white/5 rounded-2xl p-5 shadow-xl">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="w-9 h-9 rounded-full bg-emerald-500/10 flex items-center justify-center border border-emerald-500/20 text-emerald-400"><Percent size={14} /></div>
                </div>
                <div className="flex items-baseline gap-2">
                  <span className="text-[32px] font-light text-emerald-400 tabular-nums">{avgAccuracy.toFixed(1)}%</span>
          ...
          <div className="bg-[#0D1117] border border-white/5 rounded-[32px] overflow-hidden shadow-2xl">
            <div className="px-8 py-6 border-b border-white/5 flex items-center justify-between bg-white/[0.01]">
              <div className="flex gap-4">
                <button onClick={() => setActiveTab("weeks")} className={cn("text-[11px] font-black uppercase tracking-widest transition-all", activeTab === "weeks" ? "text-blue-400" : "text-white/20 hover:text-white/40")}>Semanas</button>
                <button onClick={() => setActiveTab("divergences")} className={cn("text-[11px] font-black uppercase tracking-widest transition-all", activeTab === "divergences" ? "text-blue-400" : "text-white/20 hover:text-white/40")}>Divergências</button>
              </div>
            </div>
            ...
            {activeTab === "weeks" ? (
              <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="border-b border-white/5 bg-[#080B11]/50 text-[10px] font-bold uppercase tracking-widest text-white/30">
                  ...
                  <tbody className="divide-y divide-white/[0.03]">
                    {weekGroups.map(v => {
                      const total = v.sessions.reduce((acc, s) => acc + s.validated_count, 0);
                      const acc = total > 0 ? ((total - v.divergences.length) / total) * 100 : 100;
                      return (
                        <tr key={v.week_number} onClick={() => { setSelected(v); setView("detail"); }} className="group hover:bg-white/[0.01] cursor-pointer transition-all">
                          <td className="px-6 py-4 font-bold text-white">Semana {v.week_number}</td>
                          <td className="px-6 py-4 font-mono text-white/60">{fmtFullDate(v.week_start)}</td>
                          <td className="px-6 py-4 text-center font-mono font-bold text-emerald-400">{acc.toFixed(1)}%</td>
              ...
              <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                  <thead>
                    <tr className="border-b border-white/5 bg-[#080B11]/50 text-[10px] font-bold uppercase tracking-widest text-white/30">
                      <th className="px-4 py-4 text-left">Posição</th>
                  ...
                  <tbody className="divide-y divide-white/[0.03]">
                    {consolidatedList.map(group => (
                      <tr key={group.key} className="hover:bg-white/[0.01] cursor-pointer transition-all">
                        ...
                        <td className="px-4 py-3 font-mono font-bold text-white/80">{group.code}</td>
                        <td className="px-4 py-3 text-center">
                          <span className={cn("text-[9px] font-extrabold uppercase tracking-widest px-2 py-1 rounded-full", group.resolved ? "text-emerald-400 bg-emerald-500/5 border border-emerald-500/20" : "text-amber-500 bg-amber-500/5 border border-amber-500/20")}>{group.resolved ? "Resolvido" : "Pendente"}</span>
                        </td>
                        <td className="px-4 py-3 text-right" onClick={e => e.stopPropagation()}>
                          <button onClick={() => setEditingTratativa(group)} className="p-1.5 rounded-xl text-blue-400 bg-blue-500/10 border border-blue-500/20 hover:bg-blue-500/20 transition-all"><Pencil size={12} /></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
